			...
			if(obj.value == "")
		 	{
		        ...
		        /* jQuery.validator.addMethod("mobile",function(value,element){
		 		...
		 		if(!(/^1[34578]\d{9}$/.test(obj.value))){ 
		 			 alert("手机号码格式不正确，请重输");
          	    }else{
          	    	var returnValue = $.ajax({
    		            url:"${ctx}/api/employee/checkEmployeeCall",
    		            dataType:"json",
    		            data:'employeeCall='+obj.value,
    		            async:false,
    		            cache:false,
    		            type:"post"
    		        }).responseText;
          	    	var jsonObject=eval("("+returnValue+")");  
          	    	if(jsonObject.code == "999999"){

          	    		 var company=eval("("+jsonObject.company+")");
          	    		 $("#companyName").val(company.name);
        		         $("#companyId").val(company.id);
        		         $("#companyButton").addClass('disabled'); 
        		         
        		         var office=eval("("+jsonObject.office+")");
          	    		 $("#officeName").val(office.name);
         		         $("#officeId").val(office.id);
         		         $("#officeButton").addClass('disabled'); 
         		         
         		        var employee=eval("("+jsonObject.employee+")");
         		         $("#no").val(employee.employeeId);
...
<body>
	...
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input id="oldLoginName" name="oldLoginName" maxlength="11" placeholder="请输入手机号码" type="hidden" value="${user.loginName}">
				<form:input path="loginName" htmlEscape="false" maxlength="50" class="required userName" onBlur="getLoginName(this)"/>
