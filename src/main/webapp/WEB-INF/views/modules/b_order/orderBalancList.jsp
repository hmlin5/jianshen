...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/b_order/order/">订单列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="order" action="${ctx}/b_order/order/balanc" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="order">
			<tr>
				<td><a href="${ctx}/b_order/order/form?id=${order.id}">
					${order.orderNum}
				</a></td>
				<td>
					${order.gymName}
				...
				<shiro:hasPermission name="b_order:order:edit"><td>
					<c:if test="${fns:getUser().userType ne '2'&& order.isGym eq '1'&&(order.status eq '5'||order.status eq '4')&&order.isConfirm eq '0'}">
						<a href="${ctx}/b_order/order/updateBalancStatus?id=${order.id}" onclick="return confirmx('确认要结算该订单吗？', this.href)">	结算</a>
					</c:if>
					<c:if test="${fns:getUser().userType eq '2'&&(order.status eq '5'||order.status eq '4')&&order.isGym eq '0'}">
					<a href="${ctx}/b_order/order/updateBalancStatus?id=${order.id}" onclick="return confirmx('确认该订单吗？', this.href)">确认</a>
					</c:if>

    				<%--<a href="${ctx}/b_order/order/form?id=${order.id}">修改</a>
					<a href="${ctx}/b_order/order/delete?id=${order.id}" onclick="return confirmx('确认要结算订单吗？', this.href)">删除</a>--%>
