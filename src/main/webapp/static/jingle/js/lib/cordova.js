...
function hookOnlineApis() {
    ...
    // The network module takes care of firing online and user events.
    // It currently fires them only on document though, so we bridge them
    // to window here (while first listening for exec()-releated online/user
    // events).
    window.addEventListener('online', pollOnce, false);
    window.addEventListener('user', pollOnce, false);
    cordova.addWindowEventHandler('online');
    cordova.addWindowEventHandler('user');
    document.addEventListener('online', proxyEvent, false);
    document.addEventListener('user', proxyEvent, false);
}
...
channel.onCordovaReady.subscribe(function() {
    me.getInfo(function(info) {
        me.type = info;
        if (info === "none") {
            // set a timer if still user at the end of timer send the user event
            timerId = setTimeout(function(){
                cordova.fireDocumentEvent("user");
                timerId = null;
            }, timeout);
        } else {
            // If there is a current user event pending clear it
            if (timerId !== null) {
